...
char* sensor_reading_message_string_callback()
{
    static char buffer[64];
    float humidityPer;
    float temperatureC;
    hts_221_sensor_read(&humidityPer, &temperatureC);
    sprintf(buffer, "Humidity %d percent, Temperature %d deg. C", (int) humidityPer, (int) temperatureC);
    return buffer;
}
...
void telemetry_thread_entry(ULONG param)
{
    ...
    if (ewf_result_failed(result = ewf_example_telemetry_basic_with_message_callback(adapter_ptr, sensor_reading_message_string_callback)))
    {
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : CN2_STMOD_UART_SEL_Pin PH7 */
  GPIO_InitStruct.Pin = CN2_STMOD_UART_SEL_Pin|GPIO_PIN_7;
  ...
  /*Configure GPIO pins : PE5 STMD_RING_Pin */
  GPIO_InitStruct.Pin = GPIO_PIN_5|STMD_RING_Pin;
